...
router.put('/settings/:sub/:email', async (req, res) => {
	try {
		const [sub, created] = await Subs.findOrCreate({
			where: { email: req.params.email, category: req.params.sub },
		});
